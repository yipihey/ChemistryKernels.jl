...
# Load rates_cmb (k27/k28, in this package) in an isolated module; the CMB-Compton
...
@testset "rates_cmb literal" begin
    ...
    for z in (10.0, 50.0, 300.0, 1000.0)
        @test isapprox(EmissionKernels.comp1_cmb(z), EmissionKernels.COMPA * (1.0 + z)^4; rtol=1e-12)
    end
    # Verify the COMPA constant itself matches the grackle value (rate_functions.c:1312)
    @test EmissionKernels.COMPA == 5.65e-36
